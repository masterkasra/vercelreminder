...
// { expired: آدرس اشتراک‌های منقضی‌شده، delivered: تعداد ارسال موفق به سرویس push، failed: خطاهای دیگر }
export async function sendPush(subs, payload, host) {
  if (!subs.length) return { expired: [], delivered: 0, failed: 0 };
  ...
  await Promise.all(subs.map(async sub => {
    try {
      ...
      else { failed++; console.error('Push error', err.statusCode, err.body || err.message); }
    }
  }));
  return { expired, delivered, failed };
}
